...
const CONFIG = {
    ...
    WORLD_SIZES: {
        ...
        medium: { width: 64, height: 64, name: "Средний" },
        large: { width: 128, height: 128, name: "Большой" }
    },
...
let gameState = "start"; // start, create, loading, playing, paused
let worlds = [];
let blocks = [];
let lastTime = 0;
let fps = 60;
...
function showPreloader() {
    ...
    preloader.innerHTML = `
        <div class="preloader-content">
            ...
            <div class="preloader-text">Загрузка ${CONFIG.GAME_TITLE}...</div>
...
function showScreen(screenName) {
    ...
    if (screen) {
        ...
        setTimeout(() => {
            screen.style.animation = 'screenFadeIn 0.3s ease';
        }, 10);
    }
}
...
function setupJoystick() {
    ...
    function updateJoystick(touch) {
        ...
        const maxDistance = 35; // Максимальное смещение ручки
...
function generateWorld() {
    ...
    const worldSize = document.querySelector('.size-option.active')?.dataset.size || 'medium';
